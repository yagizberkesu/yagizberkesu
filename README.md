<div align="center">

# Merhaba, ben Yağız Berke Su 👋

### Bilgisayar Mühendisliği Öğrencisi · Yazılım Geliştirici

Kullanışlı, modern ve kullanıcı deneyimine önem veren uygulamalar geliştiriyorum.  
Web, mobil uygulama ve interaktif oyun projeleri üzerinde çalışıyorum.

[![GitHub](https://img.shields.io/badge/GitHub-yagizberkesu-181717?style=for-the-badge&logo=github)](https://github.com/yagizberkesu)

</div>

---

## 🚀 Projelerim

### 🍿 NextWatch 🚧

Arkadaşlarla ya da sevgilinle "bu akşam ne izlesek?" tartışmasını bitiren, birlikte film/dizi kararı verme ve takip uygulaması.

2-8 kişilik parti kurup ortak izleme listesi oluşturuyor, öneri veriyor, oylayıp karar veriyor; birlikte izlenen dizilerde bölüm ilerlemesini senkronlayıp spoiler'sız notlar bırakıyorsunuz.

**Öne çıkan özellikler:**

- Davet linkiyle 2-8 kişilik parti kurma
- Film/dizi önerme, 4'lü tepki sistemiyle oylama, otomatik "ortak aday" belirleme
- "Bu akşam ne izliyoruz?" karar kartı: platform/süre/tür/ruh haline göre filtreleyip 3 seçenek çıkarma + veto usulü karar çarkı
- Dizilerde üye başına bölüm ilerlemesi, senkron durumu ve spoiler kilitli bölüm notları
- "Nerede izlenir" (TMDB/JustWatch) rozetleri
- Kişisel izleme listesi: durum takibi, toplu düzenleme, yeni bölüm/sezon takvimi
- Uygulama içi bildirim merkezi, TR/EN dil ve açık/koyu tema desteği

> Şu an aktif geliştirme aşamasında; kaynak kodu henüz özel (private) repoda.

[![Canlı Site](https://img.shields.io/badge/Canlı_Site-NextWatch-F7C419?style=for-the-badge&logo=vercel&logoColor=black)](https://nextwatch-ten.vercel.app/)
![Geliştirmede](https://img.shields.io/badge/Durum-Geliştirmede-orange?style=for-the-badge)

---

### 🎓 SAÜScript Plus

Sakarya Üniversitesi öğrencilerinin ders notlarını ve akademik durumlarını daha kolay takip edebilmesi için geliştirdiğim kapsamlı bir not simülatörü.

Transkript verilerini cihaz üzerinde analiz eder; farklı harf notlarının genel ortalamaya etkisini, dönem planlarını ve final hedeflerini hesaplamaya yardımcı olur.

**Öne çıkan özellikler:**

- SABİS transkript PDF’sini tarayıcı içinde analiz etme
- Kopyala-yapıştır veya elle ders girişi
- Genel ortalama ve dönem ortalaması simülasyonu
- Tekrar alınan derslerin ortalamaya etkisini hesaplama
- Güz ve bahar dönemlerine göre ders planlama
- Vize, final, ödev, proje ve diğer katkı puanlarını takip etme
- Hedef harf notu için gerekli final notunu hesaplama
- Ders bazında genel ortalama etkisi
- JSON veri yedekleme ve geri yükleme
- Karanlık tema ve mobil uyumlu arayüz
- Verilerin yalnızca kullanıcının cihazında saklanması

> SAÜScript Plus’ın Sakarya Üniversitesi veya SABİS ile resmî bağlantısı yoktur. Sonuçlar bilgilendirme amaçlıdır.

[![Canlı Site](https://img.shields.io/badge/Canlı_Site-SAÜScript_Plus-2396F3?style=for-the-badge&logo=githubpages&logoColor=white)](https://yagizberkesu.github.io/sauscript/)

---

### 🎲 Tavla

Mobil ve masaüstü cihazlarda oynanabilen, modern web tabanlı bir backgammon oyunu.

Klasik tavla mekaniklerini sosyal özelliklerle birleştiren interaktif bir oyun deneyimi sunar.

**Öne çıkan özellikler:**

- Tam fonksiyonel tavla oyun mekanikleri
- Gerçek zamanlı sohbet
- Emoji reaksiyonları
- Oyun durumu görünümü
- Chat peek özelliği
- Okunmamış mesaj rozeti
- Mobil ve masaüstü uyumlu responsive tasarım
- Modern ve kullanıcı dostu arayüz

[![Oyunu Aç](https://img.shields.io/badge/Oyunu_Aç-Tavla-43A047?style=for-the-badge&logo=githubpages&logoColor=white)](https://yagizberkesu.github.io/tavlays/)

---

### 🧩 Puzzle App

Expo ve React Native kullanılarak geliştirilmiş, kullanıcının kendi görselleriyle yapboz oluşturabildiği mobil jigsaw oyunu.

Kullanıcı galeriden bir görsel seçebilir, zorluk seviyesini belirleyebilir ve parçaları sürükleyerek yapbozu tamamlayabilir.

**Öne çıkan özellikler:**

- Galeriden görsel seçerek kişisel yapboz oluşturma
- 25 ile 1024 arasında parça seçeneği
- Gerçekçi klasik `tab/blank` jigsaw geometrisi
- Sürükle-bırak ile parça yerleştirme
- Birbirine uyan parçaları otomatik birleştirme
- Parçaları çerçeveye otomatik oturtma
- Board üzerinde pinch-to-zoom ve pan hareketleri
- Sesli ve titreşimli birleşme geri bildirimleri
- Kilometre taşı bildirimleri
- Yatay kaydırılabilen parça tepsisi
- Parçaların board’a taşınırken gerçek boyutuna geçiş animasyonu
- AsyncStorage ile kayıtlı yapbozlara kaldığın yerden devam etme

---

## 🛠️ Kullandığım Teknolojiler

<div align="center">

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white)
![React Native](https://img.shields.io/badge/React_Native-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![Expo](https://img.shields.io/badge/Expo-000020?style=flat-square&logo=expo&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=flat-square&logo=firebase&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=flat-square&logo=vite&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub Pages](https://img.shields.io/badge/GitHub_Pages-222222?style=flat-square&logo=githubpages&logoColor=white)

</div>

---

## 📊 GitHub İstatistiklerim

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=yagizberkesu&show_icons=true&theme=github_dark&hide_border=true&locale=tr)

![Most Used Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=yagizberkesu&layout=compact&theme=github_dark&hide_border=true&locale=tr)

</div>

> Özel repolardaki çalışmalar kaynak kod veya proje ayrıntıları paylaşılmadan katkı grafiğinde anonim olarak gösterilebilir.

---

## 💡 Şu Anda

- Web ve mobil uygulama geliştirme konusunda kendimi geliştiriyorum.
- Kullanıcı deneyimi güçlü ve günlük hayatta işe yarayan projeler üretiyorum.
- React, TypeScript, React Native ve oyun mekanikleri üzerinde çalışıyorum.
- Yeni fikirleri çalışan ürünlere dönüştürmeyi seviyorum.

---

<div align="center">

### Kod yazmak yalnızca bir şeyleri çalıştırmak değil, iyi bir deneyim tasarlamaktır.

**Yağız Berke Su**

</div>
